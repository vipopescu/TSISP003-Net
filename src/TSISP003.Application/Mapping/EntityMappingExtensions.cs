...
namespace TSISP003.Application.Mapping;
...
public static class EntityMappingExtensions
{
    ...
    public static SignSetTextFrameDto AsDto(this SignSetTextFrame signSetTextFrame)
    {
        ...
        try
        {
            asciiText = HexToAscii(signSetTextFrame.Text);
        }
        catch
        {
        }
    ...
    public static SignSetTextFrame AsEntity(this SignSetTextFrameDto signSetTextFrameDto)
    {
        ...
        try
        {
            hexText = AsciiToHex(signSetTextFrameDto.Text);
        }
        catch
        {
        }
        ...
        return new SignSetTextFrame
        {
            ...
            CRC = PacketCRCushort(Encoding.ASCII.GetBytes(hexText))
        };
    }
    ...
    {
        string hexText = AsciiToHex(extendedTextFrameDto.Text);
        return new SignSetTextFrame
        {
            FrameID = frameid,
            Revision = revision,
    ...
    // Private helper methods (moved from Functions utility class since they're needed for mapping)
    ...
    private static ushort CRCGenerator(ushort data, ushort accum)
    {
    ...
    private static ushort PacketCRCushort(byte[] packetBytes)
    {
